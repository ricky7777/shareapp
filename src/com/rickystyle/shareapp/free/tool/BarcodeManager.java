...
public class BarcodeManager {
	...
	DebugTool.printILog("isBarcodeExists():" + isBarcodeExists());
	if (!isBarcodeExists()) {
	    saveBarcodeImg(null);
	}
	...
	try {
	    ...
	    DebugTool.printVLog("getBarcodeInFolderURL():" + getBarcodeInFolderURL() + ",isSuccess:" + isSuccess);
	    fOut.flush();
	    if (fOut != null)
		fOut.close();

	} catch (Exception e) {
	    DebugTool.printELog("get url fail:" + packageName);
	}
    }
	...
	try {
	    ...
	    DebugTool.printELog("get barcode Img fail:" + text);
	}
	return bm;
    }
     ...
     * @param mPm
     */
    public void addAppBookmark(ApplicationInfo info, Context context, PackageManager mPm) {
	...
	if (!isBarcodeIconExists()) {
		...
		DebugTool.printVLog("getBarcodeIconURL:" + getBarcodeIconURL() + ",isSuccess:" + isSuccess);
		fOut.flush();
		if (fOut != null)
		    fOut.close();
	    } catch (Exception e) {
		DebugTool.printELog("save bookmark error:" + e.getMessage());
	    }
	}
	...
	DebugTool.printVLog("add bookmark cost:" + (System.currentTimeMillis() - start));
